    ...
    if interview_mode == "precise":
        ...
        if is_complete:
            ...
            for h in state["history"]:
                if h["role"] == "agent":
                    formatted_history.append(f"Konsultantas: {h['text']}")
                elif h["role"] == "user":
                    formatted_history.append(f"Klientas: {h['text']}")
...
async def finalize_session(
    ...
    Optionally includes contact info in report header.
    """
    # Load session
    result = await db.execute(
    ...
    if request and request.contact_info:
        state["contact_info"] = request.contact_info.model_dump()
        await db.execute(
            text("UPDATE sessions SET state = :state WHERE session_id = :id"),
            {"state": json.dumps(state), "id": session_id},
        )
