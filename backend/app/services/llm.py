"""
LLM Service - Gemini + Anthropic Fallback with Translation
...
def _call_llm_with_fallback(prompt: str) -> str:
    ...
    try:
        ...
        error_str = str(e)
        if "429" in error_str or "RESOURCE_EXHAUSTED" in error_str or "quota" in error_str.lower() or "503" in error_str or "UNAVAILABLE" in error_str or "overloaded" in error_str.lower():
            print(f"Gemini unavailable, falling back to Anthropic: {error_str[:100]}")
            try:
...
async def extract_slots(agent_state, user_answer):
